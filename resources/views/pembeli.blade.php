...
<body>
  ...
  <div class="info">
    ...
    @endif
      <a href="/buatpembeli"> <button class="btn btn-info"> Buat Baru</button> </a>
      <table class="table table-hover">
        ...
        <tbody>
          @foreach ($pembeli as $no => $u)
          <tr>
            <th scope="row">{{ $pembeli->firstitem() + $no }}</th>
            <td>{{ $u->namapembeli }}</td>
            <td>{{ $u->nohp }}</td>
            <td>{{ $u->bond }}</td>
            <td>
              <a href="editpembeli/{{ $u->id }}" class="btn btn-warning" method="POST">
                Ubah</a>
            </td>
            <td>
              <form action="{{ route('pembeli.destroy', $u->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                @csrf
                @method('DELETE')
